...
fuser -k 3000/tcp 4400/tcp 2>/dev/null || true
sleep 1
...
for _ in $(seq 1 120); do
  if curl -sf "http://localhost:4400/api/v1/health" >/dev/null 2>&1; then
    break
  fi
  sleep 3
done
...
echo "==> Smoke checks"
curl -sf "http://localhost:4400/api/v1/health" | head -c 200
echo ""
curl -sf -H "X-Forwarded-Host: wufud.localhost" "http://localhost:4400/api/v1/public/context" | head -c 200
echo ""
curl -sf -o /dev/null -w "frontend HTTP %{http_code}\n" "http://localhost:3000/"

echo "==> Done. Platform: http://wufud.localhost:3000  API: http://localhost:4400/api/v1/health  Mailpit: http://localhost:8025"
